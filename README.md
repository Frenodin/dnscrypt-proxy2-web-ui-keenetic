...
Лёгкая веб-панель для управления [dnscrypt-proxy2](https://github.com/DNSCrypt/dnscrypt-proxy) на роутерах Keenetic (Entware). Написана на чистом Python 3 (стандартная библиотека, без Flask/pip) — специально для устройств с ограниченными ресурсами.

Решает практическую проблему: dnscrypt-proxy2 настраивается через `dnscrypt-proxy.toml`, и без веб-интерфейса каждое изменение (смена ODoH-relay, target-сервера, кеша) требует ручного редактирования файла через SSH и рестарта сервиса вслепую. Эта панель даёт то же самое из браузера, с проверкой конфига пе
...
- Панель слушает `0.0.0.0` — доступна с любого устройства в вашей локальной сети. Не открывайте порт 8091 на WAN/интернет без дополнительной защиты (VPN, reverse proxy с TLS).
...
- `panel-auth.json` хранит пароль в открытом виде, доступ к файлу ограничен правами `chmod 600`. Не коммитьте этот файл в git.
